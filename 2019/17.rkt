...
(define th (make-runner (file->string "17.rktd")))

(define str (list->string (for/list ([val (in-producer thread-receive 'done)])
                            (integer->char val))))
(define scaffolding-locs
...
;; 1
(check-eq?
(for/sum ([loc (in-list scaffolding-locs)]
           #:when (for/and ([neighbor (in-list (map (curry + loc) (list 1 -1 +i -i)))])
                    (memv neighbor scaffolding-locs)))
  (* (real-part loc) (imag-part loc)))
7280)
